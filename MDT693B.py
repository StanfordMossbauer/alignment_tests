########################################
# Stolen from https://github.com/claudioarena/mdt69x
...
## Implementing it implies change to the code, to parse the echo response
    ...
    def get_voltage(self, channel):
        if isinstance(channel, str):
            channel = dict(x=0, y=1, z=2)(channel)
        value = self._send_query(self._voltage_commands_get[channel])
        return self._response_to_float(value)

    def get_voltages(self):
        return np.array([self.get_voltage(i) for i in range(3)])

    def set_voltage(self, voltage, channel):
        if isinstance(channel, str):
            channel = dict(x=0, y=1, z=2)(channel)
        return self._send_command(self._voltage_commands_set[channel], voltage)

    def set_voltages(self, voltages):
        return [self.set_voltage(voltages[i], i) for i in range(3)]
